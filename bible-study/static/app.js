...
function goPrev() {
  const index = state.books.findIndex((b) => b.slug === state.bookSlug);
  if (state.chapter <= 1 && index <= 0) return;
...
function goNext() {
  const book = currentBook();
  const index = state.books.findIndex((b) => b.slug === state.bookSlug);
  if (state.chapter >= book.chapters && index >= state.books.length - 1) return;
...
function updateClock() {
  ...
  if (els.datetimeDate) {
    els.datetimeDate.textContent = `${year}.${month}.${date}(${dayName})`;
  }
  if (els.datetimeTime) {
    els.datetimeTime.textContent = `${hours}:${minutes}:${seconds}`;
  }
}
